...
namespace GeneticPetri.Genetic
{
    ...
    /// </remarks>
    /// 
    public class PetriChromosome : ChromosomeBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="PetriChromosome"/> class.
        /// </summary>
        /// 
        ...
        /// 
        public PetriChromosome(PetriChromosome source)
        {
        ...
        {
            var chromosome = new PetriChromosome(chromosomeGenerator, mutationMultiplierGenerator, mutationAdditionGenerator, length);
        ...
        {
            return new PetriChromosome(this);
        }
        ...
        {
            PetriChromosome p = (PetriChromosome)pair;
            var rand = Generator.Random;

            double fator = rand.NextDouble();

            double[] p1 = new double[length];
            double[] p2 = new double[length];

            Array.Copy(this.val, p1, length);
            Array.Copy(p.val, p2, length);

            //Cruzamento de valores
            for (int i = 0; i < length; i++)
            {
                this.val[i] = p1[i] * fator + p2[i] * (1.0 - fator);
                p.val[i] = p1[i] * (1.0-fator) + p2[i] *  fator;
